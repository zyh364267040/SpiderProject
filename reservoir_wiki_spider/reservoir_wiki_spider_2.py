...
def save_data(lack_name, lack_detail_trs, data_list):
    ...
    for lack_detail_tr in lack_detail_trs:
        ...
        zuobiao = lack_detail_tr.xpath('.//a/span[1]/span//text()')
        if zuobiao:
            print(zuobiao)
            zuobiao = ''.join(zuobiao[:2])
...
def data_to_excel(data_list):
    ...
    if os.path.exists('wiki2.xlsx'):
        wb = openpyxl.load_workbook('wiki2.xlsx')
        sheet = wb.active
        i = sheet.max_row + 1
        i = 401
    else:
    ...
    for data in data_list:
        ...
        sheet[f'AD{i}'] = data.get('xingcheng', '')
        sheet[f'AE{i}'] = data.get('xianzhuang', '')
...
def main():
    ...
    data_list = []
    # 所有水库页url
    lack_url = 'https://www.wiki.zh-cn.nina.az/%E4%B8%AD%E5%9B%BD%E5%A4%A7%E9%99%86%E5%A4%A7%E5%9E%8B%E6%B0%B4%E5%BA%93%E5%88%97%E8%A1%A8.html'
    ...
    all_lack = len(trs) - 1
    for num, tr in enumerate(trs[400:]):
        # 获取每一个湖的详情url
        href = tr.xpath('./th/a/@href')
        lack_name = tr.xpath('./th/a/text()')
        if href:
            href = urljoin(lack_url, href[0])
            print(f'开始获取第{num+1}个湖,{lack_name}的信息,共计{all_lack}个...')
            # print(href)
            # 发送请求,获取湖详情数据
