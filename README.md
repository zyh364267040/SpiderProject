...
| `indonesia_population_wiki_spider/` | Wikipedia 印度尼西亚行政区与人口数据采集案例，解析省、市县、下级地区及人口字段 |
| `china_college_info_spider/` | 中国高校公开信息异步采集案例，解析学校层次、排名、地区和招生办公开联系方式 |
...
### 23. 印度尼西亚行政区人口采集：indonesia_population_wiki_spider

`indonesia_population_wiki_spider` 案例从英文 Wikipedia 的印度尼西亚县市列表出发，访问行政区详情页，解析省、市/县、下级地区及不同年份的人口字段，并写入 Excel。整理时保留三个历史版本，历史结果、失败记录和访问会话信息未提交到仓库。

这个案例体现了：

- Wikipedia 行政区列表与详情页联动采集
- 省、市/县、下级地区层级结构解析
- 不同年份人口字段识别
- XPath 表格解析和 Excel 写入
- 多版本代码整理与历史 Cookie 清理

### 24. 中国高校公开信息异步采集：china_college_info_spider

`china_college_info_spider` 案例通过中国教育在线公开数据接口批量获取高校基础信息、学校层次、国内外排名、地区和招生办公开联系方式。整理后的版本使用共享 `ClientSession`、限制并发、请求重试、字段容错、断点续采和统一 CSV 写入，避免旧版的高并发和协程同时写文件问题。
...
│   ├── README.md
│   ├── indonesia_population_wiki_spider_1.py
│   ├── indonesia_population_wiki_spider_2.py
│   └── indonesia_population_wiki_spider_3.py
└── china_college_info_spider/
    ├── README.md
    ├── china_college_info_spider.py
    └── requirements.txt
```
